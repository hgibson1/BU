//e4.1 *****************************************************************************/
// gcc -msse4.1 -O1 -o test_dot8_new test_dot8_new.c -lrt -lm
//
...
#define OPTIONS 8   // SET TO 7 with new function
#define IDENT 1.0
...
main(int argc, char *argv[])
{
  ...
  void doti(vec_ptr v0, vec_ptr v1, data_t *dest);
  ...
  for (i=0; i<ITERS; i++) {
      ...
      set_vec_length(v1, BASE+(i+1)*DELTA);
      clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time1);
      doti(v0, v1, data_holder);
      clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time2);
      answer[OPTION][i] = *data_holder;
      time_stamp[OPTION][i] = diff(time1, time2);
  }  
  /* output times */
  //printf("\nsize, d4, d5, d6_2, d6_5,  d8, d8_2, d8_4, doti");  
  for (i = 0; i < ITERS; i++) {
    printf("\n%d, ", BASE+(i+1)*DELTA);
    for (j = 0; j < OPTIONS; j++) {
      if (j != 0) printf(", ");
...
void doti(vec_ptr v0, vec_ptr v1, data_t *dest) {
    long int i, loop =  get_vec_length(v0)/4;
    data_t *data0 = get_vec_start(v0);
    data_t *data1 = get_vec_start(v1);
    __m128 *src0 = (__m128*) data0;
    __m128 *src1 = (__m128*) data1;
    float results[4] __attribute__((__aligned__(16)));
    data_t result = (data_t) 0;
    __m128 m1;
    float init[4] __attribute__((__aligned__(16))) = {(float) data0[0]*data1[0],
                                                      (float) data0[1]*data1[1], 
                                                      (float) data0[2]*data1[2],
                                                      (float) data0[3]*data1[3]};
    __m128 m2 = _mm_load_ps(init);

    for (i=1; i<loop; i++) {
        m1 = _mm_mul_ps(*++src0, *++src1);
        m2 = _mm_add_ps(m2, m1); 
    }
    _mm_store_ps(results, m2);
    
    for (i=0; i<4; i++) {
        result += (data_t) results[i];
    }
    *dest = result;       
}
