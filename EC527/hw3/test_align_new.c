/**************************************************************/
// gcc -O0 -o test_align test_align.c -lrt
...
main(int argc, char *argv[])
{
  ...
  /* output times */
  for (k = 0; k < BOUNDARY_ALIGNMENT; k++)
    printf("\n %d, %ld, %ld", k, (long int)((double)(CPG)*(double)
      	 (GIG * time_stamp[0][k].tv_sec + time_stamp[0][k].tv_nsec)),
         (long int)((double)(CPG)*(double)(GIG*time_stamp[1][k].tv_sec + time_stamp[1][k].tv_nsec)));
